...
void BitMap::add(Atom* atom) {

}

BitMap::arraybits BitMap::doAndOfBitsIndexing(Atom* atom){
//	cout<<"ATOM SEARCH ";atom->print();cout<<endl;
	arraybits bits;
	auto index=atom->getTerm(indexingTerms[0])->getIndex();
	auto find_it=vectorOfIndexingStructure[indexingTerms[0]].find(index);
	if(find_it==vectorOfIndexingStructure[indexingTerms[0]].end())
		return {};
	bits=find_it->second;

	for(unsigned i=1;i<indexingTerms.size();i++){
		unsigned pos =indexingTerms[i];
		auto index=atom->getTerm(pos)->getIndex();
		auto find_it=vectorOfIndexingStructure[pos].find(index);
		if(find_it==vectorOfIndexingStructure[pos].end())
			return {};
		auto& termbits = find_it->second;
		bool allzero=true;
		if(bits.size()>termbits.size())
			bits.resize(termbits.size());
		for(unsigned i=0;i<termbits.size();i++){
			if(i>=bits.size())break;
			bits[i]&=termbits[i];
			if(allzero && bits[i].any())allzero=false;
		}
		if(allzero)return {};
	}
//	cout<<"VECTOR"<<endl;
...
Atom* BitMap::find(Atom* atom,const pair<SearchType,unsigned>& searchSpecification) {
	if(lastUpdate<table->size())
		update();
	arraybits bits=doAndOfBitsIndexing(atom);
	for(unsigned i=0;i<bits.size();i++){
		auto& bitset=bits[i];
		if(!bitset.any())continue;
		for(unsigned j=0;j<bitset.size();j++)
			if(bitset[j]){
				unsigned position=(i*N)+j;
				if(*((*table)[position])==*atom)
...
void BitMap::update() {
	...
	auto &aTable=*table;
	for (;lastUpdate<table->size();++lastUpdate) {
		for(auto t:indexingTerms){
			auto &bitmap = vectorOfIndexingStructure[t];
			auto& vecbitset = bitmap[aTable[lastUpdate]->getTerm(t)->getIndex()];
			unsigned bucket = lastUpdate/N;
			unsigned position =  lastUpdate % N;
			while(bucket>=vecbitset.size())vecbitset.emplace_back();
			vecbitset[bucket].set(position);
		}
	}
//	for(unsigned i=0;i<vectorOfIndexingStructure.size();i++){
//		cout<<"TABLE TERM "<<i<<endl;
//		for(auto p:vectorOfIndexingStructure[i]){
//			cout<<"\tVALUE "<<p.first<<" -> ";
//			for(unsigned j=0;j<p.second.size();j++){
//				for(unsigned k=0;k<p.second[j].size();k++){
//					unsigned pos=j*N+k;
//					if(pos>=table->size())break;
//					cout<<p.second[j][k]<<" = ";(*table)[pos]->print();cout<<" ";
//				}
//			}
//			cout<<endl;
//		}
...
GeneralIterator* BitMap::computeMatchIterator(Atom* templateAtom,
		const RuleInformation& ruleInformation,
		const pair<SearchType, unsigned>& searchSpecification, unsigned arg) {
	if(lastUpdate<table->size())
		update();
	arraybits bits=doAndOfBitsIndexing(templateAtom);

	return new BitMapIterator(bits,*table);
