...
:- valves_number(Nv), not Nv = #count{ X,Y : valve(X,Y), symm_pipe(X,Y) }.
...
:- junction(X), not tank(X), symm_pipe(X,A), symm_pipe(X,B),
		2 = #count{ Y : symm_pipe(X,Y) }, A>B, valve(X,A).
...
%lower(pipe(X,Y), pipe(W,Z)) :- pipe(X,Y), pipe(W,Z),
%		AGG = #sum { Dn,A,B : reached(pipe(A,B), broken(X,Y)), dem(A,B,Dn) },
%        AGG <= #sum { Dm,C,D : reached(pipe(C,D), broken(W,Z)), dem(C,D,Dm) }.

aux_aggr(Dn,A,B,X,Y,W,Z) :- pipe(X,Y), pipe(W,Z), reached(pipe(A,B), broken(X,Y)), dem(A,B,Dn).
aux_aggr(-Dm,C,D,X,Y,W,Z) :- pipe(X,Y), pipe(W,Z), reached(pipe(C,D), broken(W,Z)), dem(C,D,Dm).
lower(pipe(X,Y), pipe(W,Z)) :- pipe(X,Y), pipe(W,Z),
		#sum { S,A,B : aux_aggr(S,A,B,X,Y,W,Z) } <= 0.
...
%worst(pipe(X,Y)) :- pipe(X,Y), not non_worst(pipe(X,Y)).
%non_worst(pipe(X,Y)) :- pipe(X,Y), pipe(W,Z), not lower_lexico(pipe(X,Y),pipe(W,Z)).
